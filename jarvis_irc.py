...
irc.send(("USER "+ botnick +" "+ botnick +" "+ botnick +" :Jarvis au rapport !\n").encode())
...
while True:
    ...
    if len(t) > 1:
        ...
        if prefix('HELP'):
                ...
                ans("Control center disponible ici : http://ulminfo.fr:8080/hackave.ogg")
        elif prefix('CAMERA'):
                ...
                try:
                    ...
                    ret = subprocess.call([basepath+"/jarvis", "camera", str(angle_int)], stdout=subprocess.DEVNULL)
                    if ret == 0:
                    ...
                    try:
                        ...
                        with open(basepath+"/camera.alias", 'r') as fh:
                            alias = fh.readlines()
                        for line in alias:
                            if line.startswith(angle.upper()):
                        ...
                        ans("Usage : jarvis: camera ANGLE, ANGLE entre 0 et 180")
                ...
                try:
                    ...
                    V = int(t[2].strip())
                    B = int(t[3].strip())
                    if R > 255 or R < 0 or V > 255 or V < 0 or B > 255 or B < 0:
                        raise ValueError
                    add_history("lumiere "+str(R)+" "+str(V)+" "+str(B))
                    ret = subprocess.call([basepath+"/jarvis", "lumiere", str(R), str(V), str(B)], stdout=subprocess.DEVNULL)
                    if ret == 0:
                        ans("Lumière réglée à (R,V,B) = (%d,%d,%d)." % (R, V, B))
                    else:
                        ans("Je n'arrive pas à régler la lumière.")
                except ValueError:
                    ans("Usage : jarvis: lumiere R V B, R, V et B entre 0 et 255")
        elif prefix('LED'):
                t = (t[1].strip())[3:].strip().upper()
                if t in ["WTF", "JARVIS", "STROBO"]:
                    add_history("led "+t)
                    ret = subprocess.call([basepath+"/leds_wtf/led_"+t.lower()+".py"], stdout=subprocess.DEVNULL)
                    if ret == 0:
                ...
                if len(qqchose) > 2:
                    ...
                    ret = subprocess.call([basepath+"/jarvis", "dis", qqchose], stdout=subprocess.DEVNULL)
                    if ret == 0:
                        ans(""+qqchose+"")
                    else:
                ...
                if t[3:].strip().upper().startswith("ON"):
                    add_history("atx on")
                    ret = subprocess.call([basepath+"/jarvis", "atx", "on"], stdout=subprocess.DEVNULL)
                    if ret == 0:
                        ans("ATX allumée.")
                    else:
                    ...
                    add_history("atx off")
                    ret = subprocess.call([basepath+"/jarvis", "atx", "off"], stdout=subprocess.DEVNULL)
                    if ret == 0:
                        ans("ATX éteinte.")
                    else:
                        ans("L'ATX est devenue incontrôlable !")
                ...
                try:
                    ...
                    ans(""+citations[random.randrange(0, len(citations))]+"")
                except:
                    ans("Je perds la mémoire…")
        elif prefix("JEU"):
                ans("J'ai perdu le jeu… (poke iXce) hbar, pi, 42, et tout le reste :) (poke cphyc)")
        elif prefix("DISCLAIMER"):
                ans("Jarvis est un bot doté de capacités dépassant à la fois l'entendement et les limites d'irc. Prenez donc garde a toujours rester poli avec lui car bien qu'aucune intention malsaine ne lui a été volontairement inculquée,")
                say("JARVIS IS PROVIDED \"AS IS\", WITHOUT WARRANTY OF ANY KIND, EXPRESS OR IMPLIED, INCLUDING BUT NOT LIMITED TO THE WARRANTIES OF MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE AND NONINFRINGEMENT.")
