        ...
        if 'leds' in infos_items:
            ...
            if self.bot.current_leds is not None:
                if self.bot.current_leds == "off":
                    ...
                    to_say += "LEDs : "+greenc+self.bot.current_leds+endc+", "
            else:
                to_say += "LEDs : "+redc+"off"+endc+", "
        if 'stream' in infos_items:
            if(self.bot.oggfwd is not None and self.bot.streamh is not None and
               self.bot.oggfwd.poll() is None and self.bot.streamh.poll() is None):
